...
metadata = {
    'protocolName': 'OT3 ABR Normalize',
...
def run(protocol: protocol_api.ProtocolContext):
   ...
   # labware
    ...
    # pipette    
    p1000    = protocol.load_instrument('p1000_single_gen3', 'right', tip_racks=[tiprack_200_1]) #will need to change pipette to 'flex_1channel_1000'
    p50     = protocol.load_instrument('p50_single_gen3', 'left', tip_racks=[tiprack_50_1])  #will need to change pipette to 'flex_1channel_50'
...
#            protocol.comment("New Vol Height = "+str(round(RSBHeight,2)))
            ...
            for Mix in range(HighVolMix):
                ...
                p1000.aspirate(100)
                p1000.move_to(sample_plate.wells_by_name()[CurrentWell].bottom())
                p1000.aspirate(100)
                p1000.dispense(100)
                p1000.move_to(sample_plate.wells_by_name()[CurrentWell].center())
                p1000.dispense(100)
        ...
        #If the Required Dilution volume is <= 20ul
                ...
                p50.move_to(sample_plate.wells_by_name()[CurrentWell].bottom())
        # Mix volume <=20ul
