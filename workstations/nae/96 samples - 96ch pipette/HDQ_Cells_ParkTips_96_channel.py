...
def run(ctx):
    ...
    wash1_res = ctx.load_labware(deepwell_type, "C3",'Wash 1 reservoir').wells()[0]
    wash2_res = ctx.load_labware(deepwell_type, "B1",'Wash 2 reservoir').wells()[0]
    ...
    if starting_vol + binding_buffer_vol > 1000:
        pip.aspirate(1000, samples_m.bottom(0.3)) #original = .1
        pip.dispense(1000, waste)
    pip.return_tip()
