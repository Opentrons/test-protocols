...
metadata = {
    'protocolName': 'IDT xGen EZ 24x v4',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # pipette
    p1000 = protocol.load_instrument("p1000_multi_gen3", "left", tip_racks=[tiprack_200_1,tiprack_200_2,tiprack_200_3])
    ...
    for loop in range(RUN):
        ...
        if STEP_FRERAT == 1:
            ...
            for loop, X in enumerate(column_1_list):
                p50.pick_up_tip()
                p50.aspirate(FRERATVol, FRERAT.bottom(z=0.1))
                p50.dispense(FRERATVol, sample_plate_1[X].bottom(z=0.1))
        ...
        if STEP_LIGDECK == 1:
            ...
            protocol.move_labware(
                ...
                new_location=heatershaker_adapter,
        ...
        if STEP_PCR == 1:
            ...
            for loop, X in enumerate(column_2_list):
                p50.pick_up_tip()
                p50.mix(2,PCRVol, PCR.bottom(), rate=0.5)
                p50.aspirate(PCRVol, PCR.bottom(), rate=0.25)
                p50.dispense(PCRVol, sample_plate_1[X].bottom(), rate=0.25)
