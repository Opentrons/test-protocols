...
def run(ctx):
    ...
    def mixing(well, pip, mvol, reps=8):
        ...
        """
        center = well.top(5)
        asp = well.bottom(0.4) #original = 0.5
        disp = well.top(-8)
    ...
    def lysis(vol, source):
        ...
        for i in range(num_cols):
            ...
            for x in range(8 if not dry_run else 1):
                m1000.aspirate(tvol*.75,cells_m[i].bottom(0.4)) #original = 0.5
    ...
    def bind():
        ...
        for i, well in enumerate(samples_m):
            ...
            tiptrack(m1000,tips)
            m1000.aspirate(175,cells_m[i].bottom(0.4)) #original = 0.1
            m1000.air_gap(10)
    ...
    def dnase(vol, source):
        ...
        for i, m in enumerate(samples_m):
            ...
            for n in range(num_trans):
                ...
                m1000.aspirate(vol_per_trans, src.bottom(0.4)) #original = 0.15
