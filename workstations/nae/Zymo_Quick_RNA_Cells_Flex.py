...
metadata = {
    'protocolName': 'Zymo Magbead RNA Extraction: Cells',
...
def run(ctx):
    ...
    def bind(vol):
        ...
        h_s.open_labware_latch()
        ctx.move_labware(sample_plate,MAG_PLATE_SLOT,use_gripper=USE_GRIPPER,pick_up_offset=grip_offset("pick-up","heater-shaker",slot=4),drop_offset=grip_offset("drop","mag-plate"))
        h_s.close_labware_latch()
