...
def run(ctx):
    ...
    pip.aspirate(re_vol,samples_m.bottom(0.6)) #original = .3
    pip.dispense(re_vol,bind_res)
    if re_vol > 1000:
        dif = (starting_vol+binding_buffer_vol)-1000
        pip.aspirate(dif+50,samples_m.bottom(0.6)) #original = .1
    ...
    ctx.comment('------Removing Supernatant-----')
    pip.pick_up_tip(tips)
    pip.aspirate(bind2_vol,samples_m.bottom(0.6)) #original = .3
    ...
    for i in range(4 if not dry_run else 1):
        ...
        ctx.comment('------Removing Supernatant-----')
        pip.pick_up_tip(tips)
        pip.aspirate(wash_vol,samples_m.bottom(0.6)) #original = .3
