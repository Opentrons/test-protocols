...
def run(protocol: protocol_api.ProtocolContext):
    ...
    def p200_reuse_tip(loop):
        ...
        if TIPREUSE == 'NO':
            ...
            p200_tips += 1
        if TIPREUSE == 'YES':
            protocol.comment(str(p200_tips) + ' - ' + str(COLUMNS) + ' + ' + str(loop))
            if (p200_tips - COLUMNS + loop) <=11:
                p1000.pick_up_tip(tiprack_200_1.wells()[(p200_tips - COLUMNS + loop) * 8])
                protocol.comment('tiprack_200_1 : ' + str((p200_tips - COLUMNS + loop) * 8))
            elif (p200_tips - COLUMNS + loop) <=23:
                p1000.pick_up_tip(tiprack_200_2.wells()[(p200_tips - 12 - COLUMNS + loop) * 8])
                protocol.comment('tiprack_200_2 : ' + str((p200_tips - 12 - COLUMNS + loop) * 8))
            elif (p200_tips - COLUMNS + loop) <=35:
                p1000.pick_up_tip(tiprack_200_3.wells()[(p200_tips - 24 - COLUMNS + loop) * 8])
                protocol.comment('tiprack_200_3 : ' + str((p200_tips - 24 - COLUMNS + loop) * 8))

    def p200_drop_tip(loop):
        global TIPREUSE
        global DRYRUN
        if DRYRUN == 'NO':
            if TIPREUSE == 'YES':
...
#       ============================================================================================
        ...
        for loop, X in enumerate(column_1_list):
            ...
            p200_move_to(X, 'p300_loc1')
            p1000.dispense(RSBVol / 5, rate=0.75)
            p1000.default_speed = 5
            p200_move_to(X, 'p300_loc2')
            p1000.dispense(RSBVol / 5, rate=0.75)
            p200_move_to(X, 'p300_loc3')
            p1000.dispense(RSBVol / 5, rate=0.75)
            p200_move_to(X, 'p300_loc2')
            p1000.dispense(RSBVol / 5, rate=0.75)
            p200_move_to(X, 'p300_loc1')
