...
metadata = {
    'protocolName': 'IDT xGen EZ 96x Head PART I-III.py DRYRUN',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if MODULES == True:
        ...
        heatershaker        = 1
        reagent_plate_1     = protocol.load_labware('armadillo_96_wellplate_200ul_pcr_full_skirt','1')
    ...
    if MODULES == True:
        ...
        thermocycler    = 7
        sample_plate_1      = protocol.load_labware('armadillo_96_wellplate_200ul_pcr_full_skirt','7')
...
############################################################################################################################################
    ...
    if STEP_FRERAT == 1:
        ...
        p1000.aspirate(FRERATVol, FRERAT.bottom(z=.2)) #original = ()
        p1000.dispense(FRERATVol, sample_plate_1['A1'].bottom(z=.2)) #original = ()
        p1000.move_to(sample_plate_1['A1'].bottom(z=.2)) #original = ()
    ...
    if STEP_LIG == 1:
        ...
        p1000.dispense(LIGVol, sample_plate_1['A1'].bottom(z=.2), rate=0.25) #original = ()
        p1000.move_to(sample_plate_1['A1'].bottom(z=.2)) #original = ()
    ...
    if STEP_CLEANUP == 1:
        ...
        p1000.move_to(sample_plate_1['A1'].bottom(z=.5)) #original = .5
        ...
        p1000.pick_up_tip(tiprack_200_1['A1']) #<---------------- Tip Pickup
        p1000.move_to(sample_plate_1['A1'].bottom(z=.2)) #original = (z=0)
        ...
        TransferSup = 20
        p1000.pick_up_tip(tiprack_20_1['A1']) #<---------------- Tip Pickup
        p1000.move_to(sample_plate_1['A1'].bottom(z=.25)) #original = .25
    ...
    if STEP_POSTPCR == 1:
        ...
        p1000.aspirate(RemoveSup-100, rate=0.25)
        protocol.delay(minutes=0.1)
        p1000.move_to(sample_plate_2['A1'].bottom(z=.5)) #original = .5
        p1000.aspirate(100, rate=0.25)
        p1000.default_speed = 5
        ...
        protocol.comment('--> Removing Residual ETOH')
        p1000.pick_up_tip(tiprack_200_1['A1']) #<---------------- Tip Pickup
        p1000.move_to(sample_plate_2['A1'].bottom(z=.2)) #original = (z=0)
        p1000.aspirate(50, rate=0.25)
        p1000.default_speed = 200
        p1000.dispense(100, Liquid_trash.top(z=0))
        protocol.delay(minutes=0.1)
        ...
        TransferSup = 20
        p1000.pick_up_tip(tiprack_20_1['A1']) #<---------------- Tip Pickup
        p1000.move_to(sample_plate_2['A1'].bottom(z=.25)) #original = .25
        p1000.aspirate(TransferSup+1, rate=0.25)
