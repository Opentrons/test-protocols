...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p1000.distribute(volume = [30, 30, 30, 30, 30, 30], source = master_reservoir['A1'].bottom(z=0.5), dest = [sample_plate_1['A1'], sample_plate_1['A2'], sample_plate_1['A3'], sample_plate_1['A4'], sample_plate_1['A5'], sample_plate_1['A6']], blow_out = True)

    # Reservoir Plate Prep: 14 mL into column 5
    p1000.transfer(volume = 1750, source = master_reservoir['A1'].bottom(z=0.5), dest = reservoir['A5'].top(), blow_out = True)

    # Reagent Plate Prep: dispense 30 and 200 ul into columns 1 and 3 - total 1840 ul
    p1000.distribute(volume = [30, 200], source = master_reservoir['A1'].bottom(z=0.5), dest = [reagent_plate_1['A1'], reagent_plate_1['A3']], blow_out = True)
