...
############################################################################################################################################
    ...
    if STEP_DILUTE == 1:
        ...
        for loop, X in enumerate(column_1_list):
            ...
            p1000.dispense(98, dilution_plate[column_DIL1_list[loop]].bottom(z=0.4)) #original = .3
            p1000.dispense(95, dilution_plate[column_DIL2_list[loop]].bottom(z=0.4))#original = .3
    ...
    for loop in range(RUN):
        if STEP_MIX == 1:
            ...
            for loop, X in enumerate(column_3_list):
                ...
                p50.dispense(qPCRVol, mix_plate.wells_by_name()[X].bottom(z=0.4)) #original = .3
            ...
            tipcheck()
            p50.pick_up_tip()
            p50.aspirate(SampleVol, STD.bottom(z=0.4)) #original = .3
            p50.dispense(SampleVol, mix_plate['A1'].bottom(z=0.4)) #original = .3
        ...
        if STEP_DISPENSE == 1:
            if FORMAT == '96':
                ...
                for loop, X in enumerate(column_3_list):
                    ...
                    p50.aspirate(MixqPCRVol+2, mix_plate[X].bottom(z=0.4)) #original = .3
            ...
            if FORMAT == '384':
                ...
                for loop, X in enumerate(column_3_list):
                    ...
                    p50.pick_up_tip()
                    p50.mix(5,MixqPCRVol-5, mix_plate[X].bottom(z=1))
                    p50.aspirate(MixqPCRVol+5, mix_plate[X].bottom(z=0.4)) #original = .3
                    p50.dispense(2, mix_plate[X].bottom(z=0.4)) #original = .3
                    protocol.delay(seconds=0.2)
                    #===============================================
