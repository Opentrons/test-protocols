...
def run(ctx):
    ...
    hs_adapter          = h_s.load_adapter("opentrons_96_deep_well_adapter")
    sample_plate      = hs_adapter.load_labware("nest_96_wellplate_2ml_deep")
    ...
    tempdeck_adapter = tempdeck.load_adapter("opentrons_96_well_aluminum_block")
    #Keep elution warm during protocol
    elutionplate = tempdeck_adapter.load_labware('armadillo_96_wellplate_200ul_pcr_full_skirt')
    ...
    # load instruments
    pip = ctx.load_instrument('flex_96_channel_1000', mount='left')
    ...
    def remove_supernatant(vol,waste):
        ...
        transfer_vol = vol/x
        for i in range(x):
            pip.aspirate(transfer_vol,samples_m.bottom(0.5))
