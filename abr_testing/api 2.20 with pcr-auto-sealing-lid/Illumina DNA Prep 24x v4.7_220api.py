...
metadata = {
    'protocolName': 'Illumina DNA Prep 24x v4.7 with TC Auto Sealing Lid',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    reservoirs_need_liquid_detection = [AMPure, TAGSTOP, TWB, EtOH]
    ...
    for loop in range(RUN):
        ...
        if STEP_TAG == 1:
            ...
            for loop, X in enumerate(column_1_list):
                tipcheck()
                p50.pick_up_tip()
                p50.require_liquid_presence(TAGSTOP)
        ...
        if STEP_WASH == 1:
            ...
            for X in range(3):
                ...
                for loop, X in enumerate(column_1_list):
                    tipcheck()
                    p1000.pick_up_tip()
                    p1000.require_liquid_presence(TWB)
        ...
        if STEP_CLEANUP == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.mix(3,AMPureVol)
                p50.require_liquid_presence(AMPure)
