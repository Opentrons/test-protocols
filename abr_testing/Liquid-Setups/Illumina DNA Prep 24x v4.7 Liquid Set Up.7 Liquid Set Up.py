...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p1000.distribute(volume = [30, 30, 30, 30], source = master_reservoir['A1'].bottom(z=0.5), dest = [sample_plate_1['A1'], sample_plate_1['A2'], sample_plate_1['A3'], sample_plate_1['A4']], blow_out = True, blowout_location = 'source well', trash = False)
    
    # Reservoir Plate Prep: dispense 375, 250, 900, 900 uls into columns 1, 2, 4, and 6 - total 19400 ul
    p1000.transfer(volume = [375, 250, 900, 900], source = master_reservoir['A1'], dest = [reservoir['A1'].top(), reservoir['A2'].top(), reservoir['A4'].top(), reservoir['A6'].top()], blow_out = True, blowout_location = 'source well', trash = False)

    # Reagent Plate Prep: 66, 132, 120, 96, 5, 5, 5, 5 - total 3472 
    p1000.transfer(volume = [66, 132, 120, 96], source = master_reservoir['A1'], dest = [reagent_plate_1['A1'], reagent_plate_1['A2'], reagent_plate_1['A3'], reagent_plate_1['A4']], blow_out = True, blowout_location = 'source well', trash = False)
    p1000.distribute(volume = [5,5,5,5], source = master_reservoir['A1'], dest = [ reagent_plate_1['A7'].bottom(z=0.5), reagent_plate_1['A8'].bottom(z=0.5), reagent_plate_1['A9'].bottom(z=0.5), reagent_plate_1['A10'].bottom(z=0.5)], blow_out = True, touch_tip = True, blowout_location = 'source well', trash = False)