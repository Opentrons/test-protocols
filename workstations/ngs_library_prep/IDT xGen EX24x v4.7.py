    ...
    for loop in range(RUN):
        ...
        if STEP_FRERAT == 1:
            ...
            for loop, X in enumerate(column_1_list):
                p50.pick_up_tip()
                p50.aspirate(FRERATVol, FRERAT.bottom(z=0.7))
                p50.dispense(FRERATVol, sample_plate_1[X].bottom(z=0.7))
        ...
        if STEP_LIG == 1:
            ...
            for loop, X in enumerate(column_1_list):
                p1000.pick_up_tip()
                p1000.mix(3,LIGVol, LIG.bottom(z=0.7), rate=0.5)
                p1000.aspirate(LIGVol, LIG.bottom(z=0.7), rate=0.2)
                p1000.default_speed = 5
                p1000.move_to(LIG.top(5))
                protocol.delay(seconds=0.2)
                p1000.default_speed = 400
                p1000.dispense(LIGVol, sample_plate_1[X].bottom(z=0.7), rate=0.25)
        ...
        if STEP_PCR == 1:
            ...
            for loop, X in enumerate(column_2_list):
                p50.pick_up_tip()
                p50.aspirate(BarcodeVol, reagent_plate.wells_by_name()[barcodes[loop]].bottom(.7), rate=0.25)
            ...
            for loop, X in enumerate(column_2_list):
                p50.pick_up_tip()
                p50.mix(2,PCRVol, PCR.bottom(.7), rate=0.5)
                p50.aspirate(PCRVol, PCR.bottom(.7), rate=0.25)
                p50.dispense(PCRVol, sample_plate_1[X].bottom(.7), rate=0.25)
