...
#defining variables inside def run
    ...
    ASP_HEIGHT = ctx.params.dot_bottom
    ...
    def transfer_well_to_plate(vol2, start, end, liquid, drop_height=-20):
        if liquid == 5:
            p1000.require_liquid_presence = True
            p1000.pick_up_tip()
            ...
            for j in range(NUM_COL): 
                end_loc_gap = end[j*8] 
                if liquid == 2:
                    p1000_single.mix(2, vol*0.75, start_loc.bottom(z=ASP_HEIGHT*5), rate = 2)
                p1000_single.require_liquid_presence(start_loc)
