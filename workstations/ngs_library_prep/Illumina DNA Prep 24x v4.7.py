    ...
    for loop in range(RUN):
        ...
        if STEP_TAG == 1:
            ...
            for loop, X in enumerate(column_1_list):
                tipcheck()
                p50.pick_up_tip()
                p50.aspirate(TAGSTOPVol+3, TAGSTOP.bottom(.5))  #original = ()
                p50.dispense(3, TAGSTOP.bottom(.5))  #original = ()
                p50.dispense(TAGSTOPVol, sample_plate_1[X].bottom(.5))  #original = ()
                p50.move_to(sample_plate_1[X].bottom(.5))  #original = ()
        ...
        if STEP_WASH == 1:
            ...
            for X in range(3):
                ...
                for loop, X in enumerate(column_1_list):
                    ...
                    p1000.move_to(sample_plate_1[X].bottom(.5))  #original = ()
                    protocol.delay(minutes=0.1)
            ...
            for loop, X in enumerate(column_1_list):
                tipcheck()
                p50.pick_up_tip()
                p50.aspirate(BarcodeVol+1, reagent_plate.wells_by_name()[barcodes[loop]].bottom(z=0.5), rate=0.25)  #original = .3
                p50.dispense(1, reagent_plate.wells_by_name()[barcodes[loop]].bottom(z=0.5), rate=0.25)  #original = .3
        ...
        if STEP_CLEANUP == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.aspirate(H20Vol+5, H20.bottom(.5), rate=1)  #original = ()
                p50.dispense(5, H20.bottom(.5), rate=1)  #original = ()
