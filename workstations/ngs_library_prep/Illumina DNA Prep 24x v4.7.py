    ...
    for loop in range(RUN):
        ...
        if STEP_TAG == 1:
            ...
            for loop, X in enumerate(column_1_list):
                tipcheck()
                p50.pick_up_tip()
                p50.aspirate(TAGSTOPVol+3, TAGSTOP.bottom(.4))
                p50.dispense(3, TAGSTOP.bottom(.4))
                p50.dispense(TAGSTOPVol, sample_plate_1[X].bottom(.4))
                p50.move_to(sample_plate_1[X].bottom(.4))
        ...
        if STEP_WASH == 1:
            ...
            for loop, X in enumerate(column_1_list):
                tipcheck()
                p50.pick_up_tip()
                p50.aspirate(BarcodeVol+1, reagent_plate.wells_by_name()[barcodes[loop]].bottom(z=0.4), rate=0.25)
                p50.dispense(1, reagent_plate.wells_by_name()[barcodes[loop]].bottom(z=0.4), rate=0.25)
        ...
        if STEP_CLEANUP == 1:
            ...
            for loop, X in enumerate(column_1_list):
                ...
                p50.aspirate(H20Vol+5, H20.bottom(.4), rate=1)
                p50.dispense(5, H20.bottom(.4), rate=1)
