...
Slot B1: Nest 96 Deep Well (wash2 reservoir-700 ul  each well)
Slot B2: Nest 96 Deep Well (wash3 reservoir-1000 ul  each well)
Slot B3: Nest 1 Well Reservoir
Slot C1: Magblock
Slot C2: Nest 96 Deep Well (beads and bind reservoir- 485 ul each well)
Slot C3: Nest 96 Deep Well (wash1 reservoir-700 ul each well)
Slot D1: H-S with Machaerey-Nagel 96 Well Deepwell and DW Adapter (empty plate)
Slot D2: Nest 96 Deep Well (lysis reservoir-325 ul each well)
...
def run(ctx):
    ...
    """
    deepwell_type2 = "macherey_nagel_dwplate_2200ul"
    deepwell_type = "opentrons_96_deep_well_adapter_nest_wellplate_2ml_deep"
    ...
    sample_plate = h_s_adapter.load_labware(deepwell_type2)
    ...
    lysis_res = ctx.load_labware(deepwell_type, 'D2').wells()[0] 
    bind_res = ctx.load_labware(deepwell_type, 'C2').wells()[0] 
    wash1_res = ctx.load_labware(deepwell_type, 'C3').wells()[0]
    wash2_res = ctx.load_labware(deepwell_type, 'B1').wells()[0]
    wash3_res = ctx.load_labware(deepwell_type, 'B2').wells()[0]
