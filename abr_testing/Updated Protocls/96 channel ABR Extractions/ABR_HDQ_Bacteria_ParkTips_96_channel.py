...
Slot B1: Nest 96 Deep Well (wash1 reservoir-700 ul  each well)
Slot B2: Nest 96 Deep Well (wash2 reservoir-1300 ul each well)
Slot B3: Nest 96 Deep Well (beads and bind reservoir-440 ul each well)
Slot C1: Magblock
Slot C2: Nest 96 Deep Well (AL reservoir- 330 ul each well)
Slot C3: Nest 96 Deep Well (200 ul each well)
Slot D1: H-S with Nest 96 Well Deepwell and DW Adapter (empty plate)
Slot D2: Nest 96 Deep Well (TL reservoir- 370 ul each well)
...
def run(ctx):
    ...
    TL_plate = h_s_adapter.load_labware(deepwell_type)
    TL_samples = TL_plate.wells()[0]
    sample_plate = ctx.load_labware(deepwell_type,'C3')
    ...
    elutionplate = temp_block.load_labware('armadillo_96_wellplate_200ul_pcr_full_skirt')
    ...
    TL_res = ctx.load_labware(deepwell_type, 'D2').wells()[0]
    AL_res = ctx.load_labware(deepwell_type, 'C2').wells()[0]
    wash1_res = ctx.load_labware(deepwell_type, 'B1').wells()[0]
    wash2_res = ctx.load_labware(deepwell_type, 'B2').wells()[0]
    bind_res = ctx.load_labware(deepwell_type, 'B3').wells()[0]
