...
def run(ctx):
    ...
    for i in range(2 if not dry_run else 1):
        ...
        pip.pick_up_tip(tips)
        pip.flow_rate.aspirate = 35
        pip.aspirate(wash_vol+50,samples_m.bottom(0.3)) #original = .3
    ...
    if not dry_run:
        ...
        pip.aspirate(wash3_vol,samples_m.bottom(.3)) #original = .15
