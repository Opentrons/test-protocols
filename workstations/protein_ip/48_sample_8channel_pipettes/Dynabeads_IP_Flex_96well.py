...
def run(ctx):
    ...
    reagent_res = ctx.load_labware('nest_12_reservoir_15ml', 6, 'reagents')
    ...
    def transfer_well_to_plate(vol2, start, end, liquid, drop_height=-20):
        if liquid == 5:
            ...
            start_loc = start
            end_loc_gap = end[0]
            p1000.mix(10, vol2*NUM_COL*0.5, start_loc.bottom(z=ASP_HEIGHT), rate = 2)
            p1000.aspirate(vol2*NUM_COL, start_loc.bottom(z=ASP_HEIGHT), rate = 2)
            p1000.air_gap(10)
            p1000.dispense(10, end_loc_gap.top(z=-5))
            for j in range(NUM_COL):   
                end_loc = end[j]
                p1000.dispense(vol2, end_loc.top(z=drop_height), rate = 0.75)    
            p1000.blow_out()
            p1000.drop_tip()
